...
class AmazonJobsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page = response.css("button[data-test-id='next-page']::attr(href)").get()
        if next_page:
            self.logger.info("Following next page: %s", next_page)
            yield response.follow(next_page, callback=self.parse)
        else:
            self.logger.info("No next page URL found; check if pagination requires manual URL construction.")
